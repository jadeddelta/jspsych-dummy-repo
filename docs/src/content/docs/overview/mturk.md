...
A common use of jsPsych is to build an online experiment and find participants using [Mechanical Turk](http://www.mturk.com/). Once an experiment is available through a web server and data is being [saved on the server](data), connecting the experiment with Mechanical Turk takes only a few additional steps. jsPsych has some built-in functionality to assist with this process. 

## The jsPsych.turk module

The [jsPsych.turk](../reference/jspsych-turk) module contains functions that are relevant for experiments running on Mechanical Turk.
