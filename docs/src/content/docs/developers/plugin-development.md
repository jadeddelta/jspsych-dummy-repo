...
:::note[Automatic versioning with custom build environments]
...
Plugins can optionally support [simulation modes](../overview/simulation).
...
5. In `visual` mode, invoke the `trial()` method of the plugin and then use the artificial data to trigger the appropriate events. There are a variety of methods in the [Plugin API module](../reference/jspsych-pluginAPI) to assist with things like simulating key presses and mouse clicks.
